module BlueCarbonRegistry::LiquidityPool {
    use std::signer;
    ...
    use BlueCarbonRegistry::CarbonCreditToken::{Self, CarbonCreditToken};
    ...
    public fun initialize(account: &signer) {
        ...
        // Initialize the carbon credit token first
        CarbonCreditToken::initialize(account);

        // Get the mint and burn capabilities from CarbonCreditToken
        let pool = borrow_global_mut<LiquidityPool>(account_addr);
        let mint_cap = pool.carbon_token_mint_cap;
        let burn_cap = pool.carbon_token_burn_cap;
    ...
    public entry fun add_liquidity(
        ...
        // Store the carbon tokens in the pool
    ...
    public fun get_carbon_token_balance(addr: address): u64 {
        CarbonCreditToken::get_carbon_token_balance(addr)
    }

    /// Get APT balance for an address
    public fun get_apt_balance(addr: address): u64 {
        CarbonCreditToken::get_apt_balance(addr)
    }
